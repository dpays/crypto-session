...
module.exports = function(app, opts) {
  ...
  function encode(body) {
      try {
          ...
          console.error('dpay-crypto-session: encode error resetting session', body, err, (err ? err.stack : undefined));
  ...
  function decode(text) {
    try {
        ...
        try {
            ...
            // Already JSON
            console.log('dpay-crypto-session: Encrypting plaintext session.', jsonString)
            return text
        } catch(error2) {// debug('decode %j error: %s', json, err);
            throw new Error('dpay-crypto-session: Discarding session: ' + text)
        }
        console.error('dpay-crypto-session: decode error resetting session', text, err, (err ? err.stack : undefined));
        return {};
    }
  }
}
